<h6 align="center">Haskell Programming</h6>
<p align="center">
  <img src="https://wiki.haskell.org/wikistatic/haskellwiki_logo.png">
</p>

##### [01 All You Need is Lambda](https://github.com/evturn/haskell-programming/tree/master/01)
- [x] AllYouNeedisLambda
- [x] [What is functional programming?](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#functional-programming-1)
- [x] [What is a function?](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#functions-2)
- [x] [The structure of lambda terms](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#lambda-terms-1)
- [x] [Beta reduction](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#beta-reduction-1)
- [x] [Multiple arguments](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#multiple-arguments)
- [x] Evaluation is simplification
- [x] [Combinators](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#combinators)
- [x] [Divergence](https://github.com/evturn/haskell-programming/blob/master/01/notes.md#divergence)
- ###### Chapter Exercises

##### [02 Hello, Haskell!](https://github.com/evturn/haskell-programming/tree/master/02)
- [x] Hello,Haskell
- [x] Interacting with Haskell code
- [x] [Understanding expressions](https://github.com/evturn/haskell-programming/blob/master/02/notes.md#expressions)
- [x] [Functions](https://github.com/evturn/haskell-programming/blob/master/02/notes.md#functions)
- [x] Evaluation
- [x] Infix operators
- [x] Declaring values
- [x] [Arithmetic functions in Haskell](https://github.com/evturn/haskell-programming/blob/master/02/notes.md#arithmetic-functions)
- [x] Parenthesization
- [x] [Letandwhere](https://github.com/evturn/haskell-programming/blob/master/02/notes.md#let-and-where)
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/blob/master/02/chapter-exercises.hs)

##### [03 Strings](https://github.com/evturn/haskell-programming/tree/master/03)
- [x] Printing strings
- [x] A first look at types
- [x] Printing simple strings
- [x] [Top-level versus local definitions](https://github.com/evturn/haskell-programming/tree/master/03/top-or-local.hs)
- [x] Types of concatenation functions
- [x] [Concatenation and scoping](https://github.com/evturn/haskell-programming/tree/master/03/print-3-flipped.hs)
- [x] More list functions
- ###### Chapter Exercises

##### [04 Basic datatypes](https://github.com/evturn/haskell-programming/tree/master/04)
- [x] Basic Datatypes
- [x] [What are types?](https://github.com/evturn/haskell-programming/blob/master/04/notes.md#types)
- [x] [Anatomy of a data declaration](https://github.com/evturn/haskell-programming/blob/master/04/notes.md#data-constructors)
- [x] Numeric types
- [x] Comparing values
- [x] Go on and Bool me
- [x] [Tuples](https://github.com/evturn/haskell-programming/blob/master/04/notes.md#tuple)
- [x] Lists
- ###### Chapter Exercises

##### [05 Types](https://github.com/evturn/haskell-programming/tree/master/05)
- [x] Types
- [x] What are types for?
- [x] How to read type signatures
- [x] Currying
- [x] Polymorphism
- [x] Type inference
- [x] Asserting types for declarations
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/05/chapter-exercises)

##### [06 Typeclasses](https://github.com/evturn/haskell-programming/tree/master/06)
- [x] Typeclasses
- [x] What are typeclasses?
- [x] Back to Bool
- [x] Eq
- [x] [Writing typeclass instances](https://github.com/evturn/haskell-programming/tree/master/06/day-of-week.hs)
- [x] [Type-defaulting typeclasses](https://github.com/evturn/haskell-programming/tree/master/06/eq-instances.hs)
- [x] Ord
- [x] Enum
- [x] Show
- [x] Read
- [x] [Instances are dispatched by type](https://github.com/evturn/haskell-programming/blob/master/06/notes.md#instances-are-dispatched-by-type)
- [x] Gimme more operations
- [x] Typeclass inheritance, partial
- ###### Chapter Exercises

##### [07 More functional patterns](https://github.com/evturn/haskell-programming/tree/master/07)
- [x] Make it func-y
- [x] Arguments and parameters
- [x] Anonymous functions
- [x] [Pattern matching](https://github.com/evturn/haskell-programming/tree/master/07/pattern-matching)
- [x] [Case expressions](https://github.com/evturn/haskell-programming/tree/master/07/case-expressions.hs)
- [x] [Higher-order functions](https://github.com/evturn/haskell-programming/tree/master/07/higher-order-functions.hs)
- [x] Guards
- [x] Function composition
- [x] Point freestyle
- [x] Demonstrating composition
- ###### Chapter Exercises

##### [08 Recursion](https://github.com/evturn/haskell-programming/tree/master/08)
- [x] Recursion
- [x] [Factorial](https://github.com/evturn/haskell-programming/tree/master/08/factorial.hs)
- [x] Bottom
- [x] [Fibonacci numbers](https://github.com/evturn/haskell-programming/tree/master/08/fibonacci-numbers.hs)
- [x] Integral division from scratch
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/08/chapter-exercises)

##### [09 Lists](https://github.com/evturn/haskell-programming/tree/master/09)
- [x] Lists
- [x] The list data type
- [x] Pattern matching on lists
- [x] List’s syntactic sugar
- [x] [Using ranges to construct lists](https://github.com/evturn/haskell-programming/tree/master/09/enum-from-to.hs)
- [x] [Extracting portions of lists](https://github.com/evturn/haskell-programming/tree/master/09/poem-lines.hs)
- [x] List comprehensions
- [x] Spines and nonstrict evaluation
- [x] Transforming lists of values
- [x] Filtering lists of values
- [x] [Zipping lists](https://github.com/evturn/haskell-programming/tree/master/09/zipping-lists.hs)
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/09/chapter-exercises.hs)

##### [10 Folding lists](https://github.com/evturn/haskell-programming/tree/master/10)
- [x] Folds
- [x] Bringing you into the fold
- [x] Recursive patterns
- [x] Fold right
- [x] Fold left
- [x] [How to write fold functions](https://github.com/evturn/haskell-programming/tree/master/10/database-processing.hs)
- [x] Folding and evaluation
- [x] Scans
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/10/chapter-exercises.hs)

##### [11 Algebraic datatypes](https://github.com/evturn/haskell-programming/tree/master/11)
- [x] Algebraic datatypes
- [x] Data declarations review
- [x] Data and type constuctors
- [x] Type constructors and kinds
- [x] Data constructors and values
- [x] [What's a type and what's data?](https://github.com/evturn/haskell-programming/tree/master/11/vehicles.hs)
- [x] [Data constructor arities](https://github.com/evturn/haskell-programming/tree/master/11/data-constructor-arities.hs)
- [x] What makes these datatypes algebriac
- [x] [newtype](https://github.com/evturn/haskell-programming/tree/master/11/logic-goats.hs)
- [x] [Sum types](https://github.com/evturn/haskell-programming/tree/master/11/sum-types.hs)
- [x] [Product types](https://github.com/evturn/haskell-programming/tree/master/11/product-types.hs)
- [x] [Normal form](https://github.com/evturn/haskell-programming/tree/master/11/normal-form.hs)
- [x] [Constructing and deconstructing values](https://github.com/evturn/haskell-programming/tree/master/11/constructing-deconstructing.hs)
- [x] [Function type is exponential](https://github.com/evturn/haskell-programming/tree/master/11/the-quad.hs)
- [x] Higher-kinded datatypes
- [x] Lists are polymorphic
- [x] [Binary Tree](https://github.com/evturn/haskell-programming/tree/master/11/binary-tree.hs)
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/11/chapter-exercises)

##### [12 Signaling adversity](https://github.com/evturn/haskell-programming/tree/master/12)
- [x] Signaling adversity
- [x] How I learned to stop worrying and love Nothing
- [x] [Bleating either](https://github.com/evturn/haskell-programming/tree/master/12/eq-case-guard.hs)
- [x] Kinds, a thousand stars in your types
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/12/chapter-exercises)

##### [13 Building projects](https://github.com/evturn/haskell-programming/tree/master/13)
- [x] Modules
- [x] Making packages with Stack
- [x] Working with a basic project
- [x] Making our project a library
- [x] Module exports
- [x] More on importing modules
- [x] Making our program interactive
- [x] [do syntax and IO](https://github.com/evturn/haskell-programming/tree/master/13/hello)
- [x] [Hangman game](https://github.com/evturn/haskell-programming/tree/master/13/hangman)
- [x] Step One: Importing modules
- [x] Step Two: Generating a word list
- [x] Step Three: Making a puzzle
- [x] Adding a newtype
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/13/chapter-exercises)

##### [14 Testing](https://github.com/evturn/haskell-programming/tree/master/14)
- [x] Testing
- [x] A quick tour of testing for the uninitiated
- [x] [Conventional testing](https://github.com/evturn/haskell-programming/tree/master/14/addition)
- [x] [Enter QuickCheck](https://github.com/evturn/haskell-programming/tree/master/14/arbitrary-quick-check.hs)
- [x] [Morse code](https://github.com/evturn/haskell-programming/tree/master/14/morse)
- [x] [Kicking around QuickCheck](https://github.com/evturn/haskell-programming/tree/master/14/arbitrary-quick-check)
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/tree/master/14/word-number)
 
##### [15 Monoid, Semigroup](https://github.com/evturn/haskell-programming/tree/master/15)
- [x] Monoids and semigroups
- [x] What we talk about when we talk about algebras
- [x] Monoid
- [x] How Monoid is defined in Haskell
- [x] Examples of using Monoid
- [x] Why Integer doesn't have a Monoid
- [x] Why bother?
- [x] Laws
- [x] Different instance, same representation
- [x] [Reusing algebras by asking for algebras](https://github.com/evturn/haskell-programming/blob/master/15/optional-monoid.hs)
- [x] [Madness](https://github.com/evturn/haskell-programming/blob/master/15/madness.hs)
- [x] Better living through QuickCheck
- [x] Semigroup
- ###### [Chapter Exercises](https://github.com/evturn/haskell-programming/blob/master/15/chapter-exercises.hs)

##### [16 Functor](https://github.com/evturn/haskell-programming/tree/master/16)
- [x] Functor
- [x] What's a functor?
- [x] There's a whole lot of fmap going round
- [x] [Let's talk about ⨍, baby](https://github.com/evturn/haskell-programming/blob/master/16/higher-kinded-types.hs)
- [x] Functor Laws
- [x] The Good, the Bad, and the Ugly
- [x] [Commonly used functors](https://github.com/evturn/haskell-programming/blob/master/16/replace-experiment.hs)
- [x] [Transforming the unapplied type argument](https://github.com/evturn/haskell-programming/blob/master/16/heavy-lifting.hs)
- [x] QuickChecking Functor instances
- [x] [Exercises: Instances of Func](https://github.com/evturn/haskell-programming/blob/master/16/instances-of-func.hs)
- [ ] Ignoring possibilities
- [ ] A somewhat surprising functor
- [ ] More structure, more functors
- [ ] IO Functor
- [ ] What if we want to do something different?
- [ ] Functors are unique to a datatype
- ###### Chapter Exercises

##### 17 Applicitive
- [ ] Applicative
- [ ] Defining Applicative
- [ ] Functor vs. Applicative
- [ ] Applicative functors are monoidal functors
- [ ] Applicative in use
- [ ] Applicative laws
- [ ] You knew this was coming
- [ ] ZipList Monoid
- ###### Chapter Exercises

##### 18 Monad
- [ ] Monad
- [ ] Sorry - Monad is not a burrito
- [ ] Do syntax and monads
- [ ] Examples of Monad use
- [ ] Monad laws
- [ ] Application and composition
- ###### Chapter Exercises

##### 19 Applying structure
- [ ] Applied structure
- [ ] Monoid
- [ ] Functor
- [ ] Applicative
- [ ] Monad
- [ ] An end-to-end example: URL shortener
- [ ] That's a wrap!
- ###### Chapter Exercises

##### 20 Foldable
- [ ] Foldable
- [ ] The Foldable class
- [ ] Revenge of the monoids
- [ ] Demonstrating Foldable instances
- [ ] Some basic derived operations
- ###### Chapter Exercises

##### 21 Traversable
- [ ] The Traversable
- [ ] The Traversable typeclass definition
- [ ] sequenceA
- [ ] traverse
- [ ] So, what's traversable for?
- [ ] Morse code revisited
- [ ] Axing tedious code
- [ ] Do all the things
- [ ] Traversable instances
- [ ] Traversable Laws
- [ ] Quality Control
- ###### Chapter Exercises

##### 22 Reader
- [ ] Reader
- [ ] A new beginning
- [ ] This is Reader
- [ ] Breaking down the Functor of functions
- [ ] But uh, Reader?
- [ ] Functions have an Applicative too
- [ ] The Monad of functions
- [ ] Reader Monad by itself is kinda boring
- [ ] You can change what comes below, but not above
- [ ] You tn to see ReaderT, not Reader
- ###### Chapter Exercises

##### 23 State
##### 24 Parser combinators
##### 25 Composing types
##### 26 Monad transformers
##### 27 Non-strictness
##### 28 Basic libraries
##### 29 IO
##### 30 When things go wrong
##### 31 Final project